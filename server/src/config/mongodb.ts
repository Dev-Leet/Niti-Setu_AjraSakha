...
async function verifyAtlasVectorSearch(): Promise<void> {
  try {
    ...
    if (vectorIndex) {
      ...
      logger.warn('Vector search index not found — create via Atlas UI using server/atlas-setup/vector-index.json');
    }
  } catch (error: unknown) {
    const err = error as { codeName?: string; message: string };
    if (err.codeName === 'CommandNotSupported') {
      logger.warn('listSearchIndexes not supported — ensure you are using MongoDB Atlas');
    } else {
